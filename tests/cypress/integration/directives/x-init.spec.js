...
test(
    'x-init can be used outside of x-data',
    html` <div x-init="$el.textContent = 'foo'"></div> `,
    ({ get }) => get('div').should(haveText('foo'))
)

test(
    'changes made in x-init happen before the rest of the component',
    html`
...
test(
    'can make deferred changes with $nextTick',
    html`
...
test(
    'x-init will not evaluate expression if it is empty',
    html`
...
test(
    'component nested into x-init without x-data are not initialised twice',
    html`
