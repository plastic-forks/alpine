...
test('x-mask', [html`<input x-data x-mask="(999) 999-9999" />`], ({ get }) => {
    // Type a phone number:
...
test.skip(
    'x-mask with latently bound x-model',
    [
        html`
            <div x-data="{ value: '' }">
...
test(
    'x-mask with x-model with initial value',
    [
        html`
...
test('x-mask with a falsy input', [html`<input x-data x-mask="" />`], ({ get }) => {
...
test('x-mask with a falsy string input', [html`<input x-data x-mask="false" />`], ({ get }) => {
    get('input').type('1').should(haveValue('1'))
    get('input').type('2').should(haveValue('12'))
    get('input').type('ua').should(haveValue('12ua'))
    get('input').type('/').should(haveValue('12ua/'))
...
test(
    'x-mask with non wildcard alpha-numeric characters (b)',
...
test('x-mask:dynamic', [html`<input x-data x-mask:dynamic="'(999)'" />`], ({ get }) => {
    get('input').type('123').should(haveValue('(123)'))
})

test('$money', [html`<input x-data x-mask:function="$money" />`], ({ get }) => {
    ...
    get('input')
        .type('{leftArrow}{leftArrow}{leftArrow}{leftArrow}12')
        .should(haveValue('12,345,612,789.87'))
...
test(
    '$money swapping commas and periods',
...
test(
    '$money with different thousands separator',
    [html`<input x-data x-mask:function="$money($input, '.', ' ')" />`],
    ({ get }) => {
...
test(
    '$money works with permanent inserted at beginning',
...
test(
    '$money mask should remove letters or non numeric characters',
    [html`<input x-data x-mask:dynamic="$money" />`],
    ({ get }) => {
...
test(
    ...
    ({ get }) => {
        ...
        get('#2')
            ...
            .should(haveValue('-12.50'))
    }
)

test(
    '$money with custom decimal precision',
    [
        html`
