...
test(
    'can persist multiple components using the same property',
    [
        html`
...
test(
    'aliases do not affect other $persist calls',
    [
        html`
            <div x-data="{ show: $persist(false).as('foo') }">
                <button id="test" @click="show = true"></button>
...
test(
    ...
    ({ get, window }, reload) => {
        ...
        get('span').should(haveText('bar'))
        window()
            .its('sessionStorage._x_message')
            .should(beEqualTo(JSON.stringify('bar')))
        window().then((win) => {
...
test(
    'can persist to custom storage using an alias',
    [
        html`
...
test(
    'can persist using global Alpine.$persist within Alpine.store',
    [
        html`
            <div x-data>
...
test(
    'multiple aliases work when using global Alpine.$persist',
    [
        html`
            <div x-data>
                <input x-model="$store.name.firstName" />
