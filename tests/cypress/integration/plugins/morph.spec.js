...
test(
    'can morph components and preserve Alpine state',
    [
        html`
            <div x-data="{ foo: 'bar' }">
...
test(
    'morphing target uses outer Alpine scope',
    [
        html`
...
test(
    'can morph with HTML change and preserve Alpine state',
    [
        html`
            <div x-data="{ foo: 'bar' }">
                <button @click="foo = 'baz'">Change Foo</button>
...
test(
    'morphing an element with multiple nested Alpine components preserves scope',
    [
        html`
            <div x-data="{ foo: 'bar' }">
                <button @click="foo = 'baz'">Change Foo</button>
...
test(
    'can morph using a custom key function',
    [
        html`
            <ul>
...
test(
    'can morph using keys with existing key to be moved up',
    [
        html`
            <ul>
                <li key="1">foo<input /></li>
...
test(
    'can morph with added element before and siblings are different',
    [
        html`
            <button>
                <div>
    ...
    ({ get }, reload, window, document) => {
        ...
        get('button > div:nth-of-type(2)').should(
            haveHtml(`
                <div>second</div>
                <div data="true">third</div>
            `)
...
test(
    'can morph using different keys',
    [
        html`
            <ul>
                <li key="1">foo</li>
...
test(
    'can morph elements with dynamic ids',
    [
        html`
            <ul>
...
test(
    'can morph different inline nodes',
    [html` <div id="from">Hello <span>World</span></div> `],
    ({ get }, reload, window, document) => {
        let toHtml = html` <div id="to">Welcome <b>Person</b>!</div> `
...
test(
    'can morph with conditional markers',
    [
        html`
            <main>
                <!--[if BLOCK]> <![endif]-->
...
test(
    'can morph with flat-nested conditional markers',
    [
        html`
            <main>
                <!--[if BLOCK]> <![endif]-->
