...
test(
    'releases focus when x-if is destroyed',
    [
        html`
            <div x-data="{ open: false }">
...
test(
    'inert only applies aria-hidden once',
    [
        html`
            <div>
...
test(
    ...
    ({ get, window }, reload) => {
        window().then((win) => {
            ...
            get('#open').click()
            get('html').should(
                haveAttribute('style', `overflow: hidden; padding-right: ${scrollbarWidth}px;`)
            )
            get('#close').click()
            get('html').should(
                notHaveAttribute('style', `overflow: hidden; padding-right: ${scrollbarWidth}px;`)
            )
        })
    }
)
...
test(
    'focuses element with autofocus',
    [
        html`
            <div x-data="{ open: false }">
                <input type="text" id="1" />
...
test(
    'can disable x-trap autofocus with .noautofocus modifier',
    [
        html`
            <div x-data="{ open: false }">
                <input type="text" id="1" />
