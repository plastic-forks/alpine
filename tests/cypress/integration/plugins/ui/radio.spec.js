...
test(
    'it works using x-model',
    [
        html`
            <main
                x-data="{ active: null, access: [
            {
                id: 'access-1',
                name: 'Public access',
...
test(
    'it works without x-model/with default-value',
    [
        html`
            <main
                x-data="{ access: [
...
test(
    'cannot select any option when the group is disabled',
    [
        html`
            <main
                x-data="{ active: null, access: [
...
test(
    ...
    ({ get }) => {
        get('[group]')
            .should(haveAttribute('role', 'radiogroup'))
        ...
        get('[option="access-1"]').should(haveAttribute('tabindex', 0))
        ...
        for (i in 4) {
            ...
            get(`[label="access-${i}"]`).should(
                haveAttribute('id', `alpine-radio-label-${i + 1}`)
            )
            get(`[description="access-${i}"]`).should(
                haveAttribute('id', `alpine-radio-description-${i + 1}`)
            )
        }

        get('[option="access-1"]').click().should(haveAttribute('aria-checked', 'true'))
    }
)

test(
    '$radioOption.isActive, $radioOption.isChecked, $radioOption.isDisabled work',
    [
        html`
            <main
                x-data="{ access: [
    ...
    ({ get }) => {
        get('[option="access-1"]')
            ...
            .type('{downarrow}')
        get('[option="access-2"]').should(haveClasses(['active', 'checked']))
        get('[option="access-3"]').should(haveClasses(['disabled']))
    }
)

test(
    'can bind objects to the value',
    [
        html`
            <main
                x-data="{ active: null, access: [
